...
import { listCharacters, loadCharacter, saveCharacter, saveCharacterLight, findCharacterByName, findCharacterByNameInRealm, listAllCharacters, deleteCharacter, restoreDeletedCharacter } from './db/characters.js';
...
// 刷新 game_settings 缓存
app.post('/admin/settings-cache/refresh', async (req, res) => {
  const admin = await requireAdmin(req);
  if (!admin) return res.status(401).json({ error: '无管理员权限。' });
  try {
    const count = await reloadSettingsCache();
    res.json({ ok: true, count });
  } catch (err) {
    res.status(500).json({ error: err?.message || '刷新缓存失败' });
  }
});
...
app.get('/admin/vip/claim-limit-status', async (req, res) => {
  const admin = await requireAdmin(req);
  if (!admin) return res.status(401).json({ error: '无管理员权限。' });
  const enabled = await getVipClaimLimitEnabled();
  res.json({ ok: true, enabled });
});

app.post('/admin/vip/claim-limit-toggle', async (req, res) => {
  const admin = await requireAdmin(req);
  if (!admin) return res.status(401).json({ error: '无管理员权限。' });
  const { enabled } = req.body || {};
  await setVipClaimLimitEnabled(enabled === true);
...
app.get('/admin/state-throttle-status', async (req, res) => {
  ...
  const playerSave = {
    ...
    minIntervalMs: await getPlayerSaveMinIntervalMsDb(),
    managedMinIntervalMs: await getPlayerSaveManagedMinIntervalMs(),
    managedForceIntervalMs: await getPlayerSaveManagedForceIntervalMs(),
    ...
    forceFullEnabled: await getPlayerSaveForceFullEnabled()
  };
...
app.post('/admin/state-throttle-toggle', async (req, res) => {
  ...
  if (playerSave && typeof playerSave === 'object') {
    ...
    if (playerSave.managedForceIntervalMs !== undefined) await setPlayerSaveManagedForceIntervalMs(playerSave.managedForceIntervalMs);
    ...
    if (playerSave.forceFullEnabled !== undefined) await setPlayerSaveForceFullEnabled(playerSave.forceFullEnabled);
...
// 缓存怪物血量开关独立接口（便于单独控制）
app.get('/admin/cache-monster-health-status', async (req, res) => {
  const admin = await requireAdmin(req);
  if (!admin) return res.status(401).json({ error: '无管理员权限。' });
  const enabled = await getCacheMonsterHealthEnabled();
  res.json({ ok: true, enabled });
});

app.post('/admin/cache-monster-health-toggle', async (req, res) => {
  const admin = await requireAdmin(req);
  if (!admin) return res.status(401).json({ error: '无管理员权限。' });
  const { enabled } = req.body || {};
  const nextEnabled = enabled === true;
  await setCacheMonsterHealthEnabled(nextEnabled);
...
async function applyPlayerSaveConfigFromDb() {
  ...
  PLAYER_SAVE_MIN_INTERVAL_MS = await getPlayerSaveMinIntervalMsDb();
  PLAYER_SAVE_MANAGED_MIN_INTERVAL_MS = await getPlayerSaveManagedMinIntervalMs();
  PLAYER_SAVE_MANAGED_FORCE_INTERVAL_MS = await getPlayerSaveManagedForceIntervalMs();
  ...
  PLAYER_SAVE_FORCE_FULL_ENABLED = await getPlayerSaveForceFullEnabled();
}
...
function buildRewardBonusBreakdown(player, party) {
  ...
  const guildActive = Boolean(player?.guild);
  ...
  const guildBuildExpPct = Math.max(0, Number(player?.guild?.buildExpBonusPct || player?.guild?.buildRewardBonusPct || 0));
  const guildBuildGoldPct = Math.max(0, Number(player?.guild?.buildGoldBonusPct || 0));
  ...
  const goldSources = [];
  if (vipActive) {
    expSources.push({ key: 'vip', label: 'VIP', pct: 100 });
    goldSources.push({ key: 'vip', label: 'VIP', pct: 100 });
  }
  if (svipActive) {
    expSources.push({ key: 'svip', label: 'SVIP', pct: 100 });
    goldSources.push({ key: 'svip', label: 'SVIP', pct: 100 });
  }
  if (guildActive) {
  ...
  if (cultivationPct > 0) {
    expSources.push({ key: 'cultivation', label: '修真', pct: cultivationPct });
  }
  if (partyPct > 0) {
    expSources.push({ key: 'party', label: `队伍(${totalPartyCount}人)`, pct: partyPct });
  ...
  return {
    expMult,
    goldMult,
    expPct: Math.max(0, Math.round((expMult - 1) * 100)),
    goldPct: Math.max(0, Math.round((goldMult - 1) * 100)),
    expSources,
    goldSources
  };
}
...
function isBlockedCharacterSkillBookDrop(itemId) {
  const item = ITEM_TEMPLATES[itemId];
  if (!item) return false;
  ...
  for (const rarity of RARITY_ORDER) {
    ...
    if (Math.random() <= Math.min(1, table[rarity] * bonus)) {
      ...
      const filteredPool = pool.filter((id) => {
        const item = ITEM_TEMPLATES[id];
        if (isBlockedCharacterSkillBookDrop(id)) return false;
...
function dropLoot(mobTemplate, bonus = 1) {
  ...
  if (mobTemplate.drops) {
    mobTemplate.drops.forEach((drop) => {
      ...
      if (isBlockedCharacterSkillBookDrop(drop.id)) return;
...
let PLAYER_SAVE_MANAGED_FORCE_INTERVAL_MS = 5 * 60 * 1000; // 托管玩家定时重存
...
let PLAYER_SAVE_FORCE_FULL_ENABLED = false; // 强制全量重存
...
const managedForceSaveAt = new Map();
...
async function savePlayerNow(player, options = {}) {
  ...
  const forceFull = options.forceFull === true || PLAYER_SAVE_FORCE_FULL_ENABLED;
  if (forceFull) {
    // 数据完整性优先：每次保存都走重存（含JSON大字段）
...
async function buildState(player, options = {}) {
  ...
  return {
    ...
    stats: {
      ...
      exp_bonus_pct: bonusBreakdown.expPct,
      gold_bonus_pct: bonusBreakdown.goldPct,
      exp_bonus_sources: bonusBreakdown.expSources,
...
function managedForceSaveTick() {
  const now = Date.now();
  const managed = Array.from(players.values()).filter((p) => isManagedHostedPlayer(p));
  for (const player of managed) {
    const saveKey = getPlayerSaveKey(player);
    const last = managedForceSaveAt.get(saveKey) || 0;
    if (now - last < PLAYER_SAVE_MANAGED_FORCE_INTERVAL_MS) continue;
    managedForceSaveAt.set(saveKey, now);
    savePlayer(player, {
      immediate: true,
      dirty: ['base', 'position', 'inventory', 'warehouse', 'equipment', 'flags']
    }).catch(() => {});
  }
}

setInterval(managedForceSaveTick, 60 * 1000);
...
async function start() {
  ...
  // 加载 game_settings 到内存缓存
  try {
    const count = await loadSettingsCache();
    console.log(`[Startup] game_settings 缓存加载完成: ${count} 项`);
  } catch (err) {
    console.error('[Startup] game_settings 缓存加载失败:', err?.message || err);
  }
