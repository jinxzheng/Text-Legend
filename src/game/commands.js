  ...
  switch (cmd) {
    ...
    case 'train': {
      ...
      if (trainCount === 1) {
        ...
        send(`消耗 ${cost} 金币、修炼果 x${needFruit}。`);
        return;
      }
      ...
      send(`共 ${trainCount} 次修炼，消耗 ${totalCost} 金币、修炼果 x${needFruit}。`);
      return;
    }
    case 'cultivate':
    case 'xiuzhen':
    ...
    case '宠物': {
      ...
      if (sub === 'train' || sub === 'training' || sub === '修炼') {
        ...
        const attrArgs = parts.slice(2); // 跳过 'train <宠物ID>'，取属性和次数
